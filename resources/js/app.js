...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // ========================================================
    if (document.querySelector(".hero-media") || document.querySelector(".builder-section")) {
        ...
        const horizontalScroll = document.querySelector('.horizontal-wrapper');
        if (horizontalScroll && !document.querySelector('.hero-bg-wrapper')) { // Evitar choque con Experiencias
            const panels = gsap.utils.toArray('.panel');
            ...
            gsap.to(panels, {
                ...
                scrollTrigger: {
                    trigger: ".horizontal-scroll-container", 
                    pin: true, scrub: 1, snap: 1 / (panels.length - 1), 
                    end: () => "+=" + horizontalScroll.offsetWidth 
                }
            });
...
// Validar que estamos en la vista del recorrido
    ...
    if (document.querySelector(".customizer-track")) {
        ...
        const tlMaster = gsap.timeline({
            scrollTrigger: { trigger: track, start: "top top", end: "bottom bottom", scrub: 0.5, pin: ".customizer-visualizer" }
        });
    ...
    // ========================================================
    if (document.querySelector(".hero-bg-wrapper") && document.querySelector(".marquee-track")) {
        
        const cursorExp = document.getElementById('custom-cursor');
        
        if (cursorExp) {
            window.addEventListener('mousemove', (e) => {
                gsap.to(cursorExp, { x: e.clientX, y: e.clientY, duration: 0.1, ease: "power2.out" });
            });

            document.querySelectorAll('.cursor-hover-target').forEach(el => {
                el.addEventListener('mouseenter', () => { gsap.to(cursorExp, { scale: 4, duration: 0.3, ease: "back.out(2)" }); });
                el.addEventListener('mouseleave', () => { gsap.to(cursorExp, { scale: 1, duration: 0.3, ease: "power2.out" }); });
            });
        }
        ...
        if (heroBgExp) {
            window.addEventListener('mousemove', (e) => {
                ...
                gsap.to(heroBgExp, { x: xPos, y: yPos, duration: 1.5, ease: "power2.out" });
            });
        }

        const tlHeroExp = gsap.timeline();
        tlHeroExp.fromTo(".hero-img", { scale: 1.2 }, { scale: 1, duration: 3, ease: "power3.out" }, 0);
        tlHeroExp.to([".hero-label", ".hero-title-line"], { clipPath: "polygon(0 0%, 100% 0%, 100% 100%, 0 100%)", duration: 1.8, stagger: 0.15, ease: "expo.out" }, 0.2);
        tlHeroExp.to(".scroll-indicator", { opacity: 1, duration: 1 }, 1.5);

        gsap.fromTo(".scroll-line", { yPercent: -100 }, { yPercent: 100, duration: 1.5, ease: "power2.inOut", repeat: -1 });
        gsap.to(".marquee-track", { xPercent: -33.33, ease: "none", duration: 10, repeat: -1 });

        gsap.to([".grid-title", ".grid-desc"], {
            scrollTrigger: { trigger: ".grid-title", start: "top 85%" },
            clipPath: "polygon(0 0%, 100% 0%, 100% 100%, 0 100%)", duration: 1.5, stagger: 0.2, ease: "expo.out"
        });

        gsap.utils.toArray(".grid-card").forEach(card => {
            let img = card.querySelector(".grid-img");
            let tlCard = gsap.timeline({ scrollTrigger: { trigger: card, start: "top 90%" } });
            tlCard.to(card, { clipPath: "inset(0% 0 0 0)", duration: 2, ease: "expo.inOut" })
                  .fromTo(img, { scale: 1.5 }, { scale: 1.25, duration: 2, ease: "expo.inOut" }, "<"); 
        });

        const horizontalContainerExp = document.querySelector(".horizontal-container");
